...
if 0
  ...
  [rbm] = ld_model(...
  {'model', @Model.spatial_5_link_minimal},...
  {'debug', false});
  data = optimal_solution;
  data.q = data.q(5:end,:);
  data.qd = data.qd(5:end,:);
  data.qdd = data.qdd(5:end,:)
end
...
Fs{1} = [-1; 0; 0; 0; 0; 0; 0; 0];
Fs{2} = [0; -1; 0; 0; 0; 0; 0; 0];
...
for i = 1:numel(vd.F)
  for j = 1:numel(vd.F)
    ...
    B1_2_fun = Function('f', {rbm.States.q.sym}, {vd.B1{2}(i,j)});
    for k = 1:numel(tEven)
      B1_1_num{i,j}(k) = full(B1_1_fun(qEven(:,k)));
      B1_2_num{i,j}(k) = full(B1_2_fun(qEven(:,k)));
    end
  end
end
...
for i = 1:numel(vd.F)
  for j = 1:numel(vd.Fs)
    ...
    B2_2_fun = Function('f', {rbm.States.q.sym}, {vd.B2{2}(i,j)});
    for k = 1:numel(tEven)
      B2_1_num{i,j}(k) = full(B2_1_fun(qEven(:,k)));
      B2_2_num{i,j}(k) = full(B2_2_fun(qEven(:,k)));
    end
  end
end
...
for i = 1:numel(vd.Fs)
  for j = 1:numel(vd.F)     
    ...
    B3_2_fun = Function('f', {rbm.States.q.sym}, {vd.B3{2}(i,j)});
    for k = 1:numel(tEven)
      B3_1_num{i,j}(k) = full(B3_1_fun(qEven(:,k)));
      B3_2_num{i,j}(k) = full(B3_2_fun(qEven(:,k)));
    end        
  end
end
...
for i = 1:numel(vd.Fs)
  for j = 1:numel(vd.Fs)
    ...
    B4_2_fun = Function('f', {rbm.States.q.sym}, {vd.B4{2}(i,j)});
    for k = 1:numel(tEven)
      B4_1_num{i,j}(k) = full(B4_1_fun(qEven(:,k)));
      B4_2_num{i,j}(k) = full(B4_2_fun(qEven(:,k)));
    end        
  end
end
...
B5_2_fun = Function('f', {rbm.States.q.sym}, {vd.B5{2}});
for k = 1:numel(tEven)
  B5_1_num(k) = full(B5_1_fun(qEven(:,k)));
  B5_2_num(k) = full(B5_2_fun(qEven(:,k)));
end        

fprintf(', s')
s_1_fun = Function('f', {rbm.States.q.sym, rbm.States.dq.sym}, {vd.s{1}});
s_2_fun = Function('f', {rbm.States.q.sym, rbm.States.dq.sym}, {vd.s{2}});
for k = 1:numel(tEven)
  s_1_num(k) = full(s_1_fun(qEven(:,k), qdEven(:,k)));
  s_2_num(k) = full(s_2_fun(qEven(:,k), qdEven(:,k)));
end  
...
w_5_fun = Function('f', {rbm.States.q.sym, rbm.States.dq.sym}, {vd.w{5}});
w_6_fun = Function('f', {rbm.States.q.sym, rbm.States.dq.sym}, {vd.w{6}});
for k = 1:numel(tEven)
  ...
  w_5_num(k) = full(w_5_fun(qEven(:,k), qdEven(:,k)));
  w_6_num(k) = full(w_6_fun(qEven(:,k), qdEven(:,k)));
end  
